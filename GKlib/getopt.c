...
#define NONOPTION_P (argv[gk_optind][0] != '-' || argv[gk_optind][1] == '\0')
  ...
  if (nextchar == NULL || *nextchar == '\0') {
    ...
    nextchar =
        (argv[gk_optind] + 1 + (longopts != NULL && argv[gk_optind][1] == '-'));
  }
  ...
  if (longopts != NULL &&
      (argv[gk_optind][1] == '-' ||
       (long_only &&
        (argv[gk_optind][2] || !strchr(optstring, argv[gk_optind][1]))))) {
    char *nameend;
    struct gk_option *p;
    ...
    if (pfound != NULL) {
      ...
      gk_optind++;
      if (*nameend) {
        /* Don't test has_arg with >, because some C compilers don't allow it to
         * be used on enums.  */
        if (pfound->has_arg)
          gk_optarg = nameend + 1;
        else {
  ...
  /* Look at and handle the next short option-character.  */
  {
    ...
    if (temp[0] == 'W' && temp[1] == ';') {
      ...
      for (nextchar = nameend = gk_optarg; *nameend && *nameend != '=';
           nameend++)
        /* Do nothing.  */;
...
<tt>longopts[*opt_index].name</tt>. So you can distinguish among long options
either by the values in their val fields or by their indices. You can also
distinguish in this way among long options that set flags.
